...
Fold0 <- readMat(paste0(PredictionFolder, '/Fold_0_Score.mat'));
PredictScore_NS_Fold0 <- as.numeric(t(Fold0$Predict.Score));
Fold1 <- readMat(paste0(PredictionFolder, '/Fold_1_Score.mat'));
PredictScore_NS_Fold1 <- as.numeric(t(Fold1$Predict.Score));
...
# Fold 0
Energy_lm <- lm(PredictScore_Energy_Fold0 ~ Age_Fold0 + PredictScore_NS_Fold0 + Sex_Fold0 + Handedness_Fold0 + Motion_Fold0 + TBV_Fold0 + TotalStrength_Fold0, data = Behavior_Fold0);
...
# Fold 1
Energy_lm <- lm(PredictScore_Energy_Fold1 ~ Age_Fold1 + PredictScore_NS_Fold1 + Sex_Fold1 + Handedness_Fold1 + Motion_Fold1 + TBV_Fold1 + TotalStrength_Fold1, data = Behavior_Fold1);
...
Fig
ggsave(paste0(FigureFolder, '/AgePrediction_CorrACC_Energy_RegressNS.tiff'), width = 17, height = 15, dpi = 300, units = "cm");
