...
AgeEffect_Mat <- paste(Result_Path, '/InitialAll0_TargetFP/Energy_Gam_Age_YeoSystemLevel_RegressNodalStrength.mat', sep = '');
...
BorderColor <- c("#F5BA2E", "#7499C2", "#AF33AD", "#E443FF",
                 "#E76178", "#5091cd", "#00A131", "#EBE297");                
LineType <- c("solid", "solid", "solid", "dashed", "dashed",
              "dashed", "dashed", "solid");
Fig <- ggplot(data, aes(EffectRank, AgeEffects_Z)) + 
       geom_bar(stat = "identity", fill = c("#F5BA2E", "#7499C2", 
              "#AF33AD", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#EBE297"),
       ...
       scale_x_discrete(limits = c(1, 2, 3, 4, 5, 6, 7, 8), 
             labels = c("FP", "MT", "VS", "VA", "DM", "SC", "DA", "LM"))
Fig
ggsave(paste0(FigureFolder, '/AgeEffect_YeoLevel_Bar_RegressNodalStrength.tiff'), width = 16, height = 15, dpi = 300, units = "cm");
