...
PCorr_Test <- cor.test(Energy_WholeBrainAvg_Partial, Age_Partial);
print(Z_Value);
Energy_Gam_Age_Mat <- file.path(ResultantFolder, 'Energy_Gam_Age_WholeBrainLevel.mat');
writeMat(Energy_Gam_Age_Mat, Age_Z = Z_Value, Age_P = P_Value, Age_PCorr = PCorr_Test$estimate);
...
Energy_Gam_Cognition <- matrix(0, 232, 4);
for (i in 1:232)
{ 
  Energy_tmp <- Energy_Cognition[, i];
  # Gam analysis was used for age effect
