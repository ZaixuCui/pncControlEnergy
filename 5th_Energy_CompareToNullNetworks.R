...
RepeatQuantity <- 100;
Energy_NullNetworks = matrix(0, 946, 1);
for (i in 1:100)
{
  tmp <- readMat(paste0(Folder, '/NullNetworks/InitialAll0_TargetFP_NullNetwork_', as.character(i), '.mat'));
  Energy_NullNetworks = Energy_NullNetworks + rowMeans(tmp$Energy[,]);
}
Energy_NullNetworks = Energy_NullNetworks / RepeatQuantity;

P_PairedTtest <- t.test(Energy_Actual, Energy_NullNetworks);
P_Value <- P_PairedTtest$p.value

Energy <- rbind(as.matrix(Energy_Actual_WholeBrainAvg), Energy_NullNetworks);
...
ggsave(paste0(FigureFolder, '/Energy_TrueNullNetwork.tiff'), width = 17, height = 15, dpi = 300, units = "cm");
