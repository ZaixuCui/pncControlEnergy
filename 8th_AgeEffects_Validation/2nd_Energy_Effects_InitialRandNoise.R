...
ResultantFolder <- paste0(ReplicationFolder, '/results/InitialRandNoise_TargetFP');
...
for (i in c(1:100))
{
  Energy_Mat_Path <- paste0(Energy_Data_Folder, '/InitialRandNoise_ConstrainFP/InitialRandNoise_TargetFP_', as.character(i), '.mat');
  ...
  # Write to the file
  Energy_Gam_Age_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_Age_WholeBrainLevel_InitialRandNoise_', as.character(i), '.mat'));
  ...
  Energy_Gam_Age_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_Age_YeoSystemLevel_InitialRandNoise_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Age_YeoAvg, Energy_Gam_Age_CSV);
  Energy_Gam_Age_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_Age_YeoSystemLevel_InitialRandNoise_', as.character(i), '.mat'));
...
# Real Network 
Res_Path <- paste0(ResultsFolder, '/InitialRandNoise_TargetFP/Energy_Gam_Age_WholeBrainLevel.mat');
tmp <- readMat(Res_Path);
WholeBrain_PCorr <- tmp$Age.PCorr;
Res_Path <- paste0(ResultsFolder, '/InitialRandNoise_TargetFP/Energy_Gam_Age_YeoSystemLevel.mat');
tmp <- readMat(Res_Path);
FP_PCorr <- tmp$Age.PCorr[6];
# FP target with rand noise
WholeBrain_PCorr_InitialRandNoise <- matrix(0, 100, 1);
WholeBrain_P_InitialRandNoise <- matrix(0, 100, 1);
FP_PCorr_InitialRandNoise <- matrix(0, 100, 1);
FP_P_InitialRandNoise <- matrix(0, 100, 1);
for (i in c(1:100))
{ 
  Res_Path <- paste0(ResultsFolder, '/InitialRandNoise_TargetFP/Energy_Gam_Age_WholeBrainLevel_InitialRandNoise_', as.character(i), '.mat');
  tmp <- readMat(Res_Path);
  WholeBrain_PCorr_InitialRandNoise[i] <- tmp$Age.PCorr;
  WholeBrain_P_InitialRandNoise[i] <- tmp$Age.P;
  Res_Path <- paste0(ResultsFolder, '/InitialRandNoise_TargetFP/Energy_Gam_Age_YeoSystemLevel_InitialRandNoise_', as.character(i), '.mat');
  tmp <- readMat(Res_Path);
  FP_PCorr_InitialRandNoise[i] <- tmp$Age.PCorr[6];
  FP_P_InitialRandNoise[i] <- tmp$Age.P[6]
}

# Whole brain
# Histogram of PCorr
data <- data.frame(PCorr_InitialRandNoise = WholeBrain_PCorr_InitialRandNoise);
Fig <- ggplot(data, aes(x = PCorr_InitialRandNoise)) + geom_histogram(bins=30);
Fig <- Fig + labs(x = "", y = "") + theme_classic()
Fig <- Fig + theme(axis.text = element_text(size= 32, color = "black"))
Fig + scale_x_continuous(limits = c(-0.169, -0.163), breaks = c(-0.168, -0.166, -0.164)) + scale_y_continuous(limits = c(0, 9), breaks = c(0, 4, 8), expand = c(0, 0));
ggsave(paste0(FigureFolder, '/AgeEffect_InitialRandNoise_PCorr_WholeBrain.tiff'), width = 15, height = 15, dpi = 300, units = "cm");

# FP
# Histogram of PCorr
data <- data.frame(PCorr_InitialRandNoise = FP_PCorr_InitialRandNoise);
Fig <- ggplot(data, aes(x = PCorr_InitialRandNoise)) + geom_histogram(bins=30);
Fig <- Fig + labs(x = "", y = "") + theme_classic()
Fig <- Fig + theme(axis.text = element_text(size= 32, color = "black"))
Fig + scale_x_continuous(limits = c(-0.1742, -0.1697), breaks = c(-0.174, -0.172, -0.170)) + scale_y_continuous(limits = c(0, 9), breaks = c(0, 4, 8), expand = c(0, 0));
ggsave(paste0(FigureFolder, '/AgeEffect_InitialRandNoise_PCorr_FP.tiff'), width = 15, height = 15, dpi = 300, units = "cm");
