...
AgeEffect_Mat <- paste(Result_Path, '/InitialAll0_TargetFP/Energy_Gam_Age_YeoSystemLevel_RegressModularityQ.mat', sep = '');
...
Fig <- ggplot(data, aes(EffectRank, AgeEffects_Z)) + 
       geom_bar(stat = "identity", fill = c("#F5BA2E", "#AF33AD", 
              "#7499C2", "#FFFFFF", "#FFFFFF", "#E76178", "#E443FF", "#EBE297"),
       ...
       scale_x_discrete(limits = c(1, 2, 3, 4, 5, 6, 7, 8), 
             labels = c("FP", "VS", "MT", "SC", "DA", "DM", "VA", "LM")) +
       scale_y_continuous(limits = c(-5, 10), breaks = c(-5, 0, 5, 10));
Fig
ggsave(paste0(FigureFolder, '/AgeEffect_YeoLevel_Bar_RegressModularityQ.tiff'), width = 16, height = 15, dpi = 300, units = "cm");
