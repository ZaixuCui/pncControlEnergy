...
ResultantFolder <- paste0(ReplicationFolder, '/results/InitialAll0_TargetFPSpinRotate');
...
### Age effects #
#################
LargestSystemID <- matrix(0, 1, 100);
for (i in c(1:100))
{
  Energy_Mat_Path <- paste0(Energy_Data_Folder, '/SpinNullFPTarget/InitialAll0_TargetFPSpinRotate_', as.character(i), '.mat');
  Energy_Mat = readMat(Energy_Mat_Path);
  Energy <- Energy_Mat$Energy;
  Energy_YeoAvg <- Energy_Mat$Energy.YeoAvg;

  # Find the system with the largest control energy
  Energy_AllSubjectsAvg <- colMeans(Energy_YeoAvg);
  Energy_Sort <- sort(Energy_AllSubjectsAvg, index.return=TRUE)
  LargestSystemID[i] <- Energy_Sort$ix[8];
  ...
  # Write to the file
  Energy_Gam_Age_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_Age_WholeBrainLevel_TargetFPSpinRotate_', as.character(i), '.mat'));
  ...
  Energy_Gam_Age_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_Age_YeoSystemLevel_TargetFPSpinRotate_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Age_YeoAvg, Energy_Gam_Age_CSV);
  Energy_Gam_Age_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_Age_YeoSystemLevel_TargetFPSpinRotate_', as.character(i), '.mat'));
...
# Null Networks
WholeBrain_PCorr_TargetFPSpinRotate <- matrix(0, 100, 1);
FP_PCorr_TargetFPSpinRotate <- matrix(0, 100, 1);
for (i in c(1:100))
{ 
  Res_Path <- paste0(ResultsFolder, '/InitialAll0_TargetFPSpinRotate/Energy_Gam_Age_WholeBrainLevel_TargetFPSpinRotate_', as.character(i), '.mat');
  tmp <- readMat(Res_Path);
  WholeBrain_PCorr_TargetFPSpinRotate[i] <- tmp$Age.PCorr;
  Res_Path <- paste0(ResultsFolder, '/InitialAll0_TargetFPSpinRotate/Energy_Gam_Age_YeoSystemLevel_TargetFPSpinRotate_', as.character(i), '.mat');
  tmp <- readMat(Res_Path);
  FP_PCorr_TargetFPSpinRotate[i] <- tmp$Age.PCorr[6];
}
