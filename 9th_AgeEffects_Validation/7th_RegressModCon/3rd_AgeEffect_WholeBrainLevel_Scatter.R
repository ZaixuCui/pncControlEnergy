...
ModCon_WholeBrainAvg <- rowMeans(ModControl);
Energy_Gam_WholeBrainAvg <- gam(Energy_WholeBrainAvg ~ s(AgeYears, k=4) + ModCon_WholeBrainAvg + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior);
...
ggsave(paste(FigureFolder, '/AgeEffect_WholeBrainLevel_Scatter_RegressModControl.tiff', sep = ''), width = 17, height = 15, dpi = 300, units = "cm");
